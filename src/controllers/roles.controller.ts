...
import { fields } from "@/models/roles.model";
...
export const del: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (data.status === true) {
      throw createHttpError(BAD_REQUEST, `cannot delete ${Model} with status true.`);
    }
...
export const bulkDel: RequestHandler = async (req, res, next) => {
  try {
    ...
    const allCount = await model.count();
    const containsTrue = existingRecords.some((r) => r.status === true);
    if (existingRecords.length >= allCount || containsTrue) {
      throw createHttpError(BAD_REQUEST, `cannot delete ${Model} with status true or ${Model} must be at least have 1 data.`);
    }
